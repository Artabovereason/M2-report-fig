...
c      ei_ev=5.102d0
      ...
      close(15)
c      nh=2
      open(unit=17,file='nh.txt')
      read(17,*),nh
      close(17)
c      lh=1
      open(unit=18,file='lh.txt')
      read(18,*),lh
      close(18)
      zc=1.d0
      call photo(nh,lh,zc,ei_ev,xsec_au)
c      write(6,*)'xsec(pi a_0^2)=',xsec_au/pi
...
!
      subroutine photo(nh,lh,zc,ei_ev,xsec_au)
      implicit real*8(a-h,o-z)
!
      eau=27.2116d0
      a0=0.529d-10
      ...
      a02 = 0.280028D0
!
c      write(6,*)'nh lh',nh,lh
c      write(6,*)'zc',zc
c      write(6,*)'ei(eV)',ei_ev
!
!      ei_ev=5.102d0
! test Kevin
!
...
*
      ...
      etaf=-zc/pf
*
c      write(6,*)'pf etaf',pf,etaf
*
      if(lh.eq.0)then
      l=lh+1
      call rad(nh,lh,zc,pf,l,xr,xn)
c      write(6,*)'l xn xr=',l,xn,xr
...
c
      ...
      xsec_au=xsec*xt
!
!      write(6,*)'xsec(pi a_0^2)=',xsec_au/pi
!
      return
      stop
      end
...
! -------------------------------------------
!
      DOUBLE PRECISION FUNCTION P6S(Zn,R)
c
c Radial part of the 6s Bound wave function for the
c Coulomb potential. Hydrogen-like ions with charge
c Zn-1.
c P(r)=R(r)*r
c
      ...
      rho4=rho3*rho
      rho5=rho4*rho
      P6S = R*(1.d0/2160.d0/dsqrt(6.d0))*(720.d0-1800.d0*rho+1200.d0*rho2
     &         -300.d0*rho3+30.d0*rho4-rho5)*
     &         dexp(-rho/2.d0)*Zn**(1.5d0)
      RETURN
      END
...
! -------------------------------------------
!
      DOUBLE PRECISION FUNCTION P7S(Zn,R)
c
c Radial part of the 6s Bound wave function for the
c Coulomb potential. Hydrogen-like ions with charge
c Zn-1.
...
c
      ...
      rho5=rho4*rho
      rho6=rho5*rho
      P7S = R*(1.d0/17640.d0/dsqrt(7.d0))*(5040.d0-15120.d0*rho+12600.d0*rho2
     &         -4200.d0*rho3+630.d0*rho4-42.d0*rho5+rho6)*
     &         dexp(-rho/2.d0)*Zn**(1.5d0)
      RETURN
      END
c
c ----------------------------------------------
c
      subroutine rad(nh,lh,zc,pf,l,xr,xn)
      implicit real*8 (a-h,o-z)
...
1     continue
      ...
      if(nh.eq.1.and.lh.eq.0)PP=P1S(1.d0,r(i))
!
      if(nh.eq.2.and.lh.eq.0)PP=P2S(1.d0,r(i))
      if(nh.eq.2.and.lh.eq.1)PP=P2P(1.d0,r(i))
!
      if(nh.eq.3.and.lh.eq.0)PP=P3S(1.d0,r(i))
      if(nh.eq.3.and.lh.eq.1)PP=P3P(1.d0,r(i))
      if(nh.eq.3.and.lh.eq.2)PP=P3D(1.d0,r(i))
!
      if(nh.eq.4.and.lh.eq.0)PP=P4S(1.d0,r(i))
      if(nh.eq.4.and.lh.eq.1)PP=P4P(1.d0,r(i))
      if(nh.eq.4.and.lh.eq.2)PP=P4D(1.d0,r(i))
      if(nh.eq.4.and.lh.eq.3)PP=P4F(1.d0,r(i))
!
      if(nh.eq.5.and.lh.eq.0)PP=P5S(1.d0,r(i))
!
      if(nh.eq.6.and.lh.eq.0)PP=P6S(1.d0,r(i))
!
      if(nh.eq.7.and.lh.eq.0)PP=P7S(1.d0,r(i))
!
      w(i)=r(i)*fc(l+1)*PP*rp(i)
      wn(i)=PP*PP*rp(i)
2     continue
      xr=rint(w,2,imax,7,h)
      xn=rint(wn,2,imax,7,h)
      return
      end
